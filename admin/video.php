...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'add' || $act === 'update') {
        ...
        $thumbnail = normalizeAssetPath($_POST['thumbnail_old'] ?? '');
        $videoOld  = normalizeAssetPath($_POST['video_old'] ?? '');

        if (!empty($_FILES['thumbnail']['name']) && $_FILES['thumbnail']['error'] === UPLOAD_ERR_OK) {
            $ext = strtolower(pathinfo($_FILES['thumbnail']['name'], PATHINFO_EXTENSION));
            if (in_array($ext, ['jpg','jpeg','png','gif','webp']) && $_FILES['thumbnail']['size'] < 5*1024*1024) {
                $fname = 'video_' . time() . '_' . rand(1000,9999) . '.' . $ext;
                if (move_uploaded_file($_FILES['thumbnail']['tmp_name'], $uploadDir . $fname)) {
                    $old = $_POST['thumbnail_old'] ?? '';
                    if ($old && strpos($old, 'assets/images/video/') === 0) @unlink('../' . normalizeAssetPath($old));
        ...
        if (!empty($_FILES['video_file']['name']) && $_FILES['video_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, ['mp4','webm','ogg','mov','avi','mkv']) && $_FILES['video_file']['size'] < 200*1024*1024) {
                $fname = 'video_' . time() . '_' . rand(1000,9999) . '.' . $ext;
                if (move_uploaded_file($_FILES['video_file']['tmp_name'], $videoUploadDir . $fname)) {
                    if ($videoOld && strpos($videoOld, 'assets/videos/') === 0) @unlink('../' . $videoOld);
                    $url_video = 'assets/videos/' . $fname;
                }
            } else { $msg = 'File video tidak valid. Gunakan MP4/WebM/OGG/MOV/AVI/MKV, maks 200MB.'; $msgType = 'error'; }
        }

        if (!$url_video && $videoOld) {
            $url_video = $videoOld;
        }

        if (!$msg && $judul && !$url_video) {
            $msg = 'Masukkan URL video atau upload file video.';
            $msgType = 'error';
        }

        if (!$msg && $judul && $url_video) {
            if ($act === 'add') {
                $stmt = $conn->prepare("INSERT INTO video_terkait_db (judul, url_video, thumbnail) VALUES (?,?,?)");
                $stmt->bind_param("sss", $judul, $url_video, $thumbnail);
                $ok = $stmt->execute(); $stmt->close();
                $msg = $ok ? 'Video berhasil ditambahkan!' : 'Gagal menambahkan video.';
                $msgType = $ok ? 'success' : 'error';
            } else {
                $id = (int)($_POST['id'] ?? 0);
                $stmt = $conn->prepare("UPDATE video_terkait_db SET judul=?,url_video=?,thumbnail=? WHERE id=?");
                $stmt->bind_param("sssi", $judul, $url_video, $thumbnail, $id);
                $ok = $stmt->execute(); $stmt->close();
                $msg = $ok ? 'Video berhasil diupdate!' : 'Gagal mengupdate video.';
                $msgType = $ok ? 'success' : 'error';
            }
            if ($ok ?? false) { header("Location: video.php?msg=" . urlencode($msg) . "&type=$msgType"); exit; }
        }
    }
    ...
    elseif ($act === 'delete') {
        ...
        if ($id) {
            $res = $conn->query("SELECT thumbnail, url_video FROM video_terkait_db WHERE id=$id");
            if ($res && $row = $res->fetch_assoc()) {
                if ($row['thumbnail'] && strpos($row['thumbnail'], 'assets/images/video/') === 0) @unlink('../' . $row['thumbnail']);
                if ($row['url_video'] && strpos($row['url_video'], 'assets/videos/') === 0) @unlink('../' . $row['url_video']);
            }
            $conn->query("DELETE FROM video_terkait_db WHERE id=$id");
            header("Location: video.php?msg=" . urlencode('Video berhasil dihapus.') . "&type=success"); exit;
        }
    }
}
...
if (isset($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    $res = $conn->query("SELECT * FROM video_terkait_db WHERE id=$id");
    if ($res && $res->num_rows > 0) { $editData = $res->fetch_assoc(); $showForm = true; }
}
if (isset($_GET['add'])) { $showForm = true; }

$videoList = [];
$res = $conn->query("SELECT * FROM video_terkait_db ORDER BY id DESC");
if ($res) { while ($row = $res->fetch_assoc()) { $videoList[] = $row; } }
$conn->close();
?>
<!DOCTYPE html>
<html lang="id">
<head>
  ...
  <title>Kelola Video – Admin BENGPUSKOMLEKAD</title>
  ...
  <style>
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .img-preview { width:200px; height:112px; object-fit:cover; border-radius:8px; border:1px solid rgba(201,168,76,0.2); margin-top:8px; display:none; }
    ...
    .td-thumb { width:100px; height:56px; object-fit:cover; border-radius:6px; border:1px solid rgba(201,168,76,0.15); }
    .td-no-img { width:100px; height:56px; background:rgba(201,168,76,0.05); border:1px dashed rgba(201,168,76,0.2); border-radius:6px; display:flex; align-items:center; justify-content:center; font-size:0.65rem; color:var(--gray-400); text-align:center; }
...
<main class="main-content">
  <div class="topbar">
    <h1>Kelola <span>Video Terkait</span></h1>
  </div>
  <div class="content-area">
    ...
    <div style="margin-bottom:24px;">
      <a href="video.php?add=1" class="btn-add">
    ...
    <?php if ($showForm): ?>
    <div class="form-card">
      <h3><?= $editData ? 'Edit Video' : 'Tambah Video Baru' ?></h3>
      <form method="POST" enctype="multipart/form-data" id="formVideo">
        <input type="hidden" name="act" value="<?= $editData ? 'update' : 'add' ?>">
        <?php if ($editData): ?>
        <input type="hidden" name="id" value="<?= $editData['id'] ?>">
        <input type="hidden" name="thumbnail_old" value="<?= htmlspecialchars($editData['thumbnail']) ?>">
        <?php endif; ?>

        <div class="form-group">
          <label>Judul Video</label>
          <input type="text" name="judul" required placeholder="Masukkan judul video..." value="<?= htmlspecialchars($editData['judul'] ?? '') ?>">
        </div>

        <div class="form-group">
          <label>URL Video / File Video Lokal (Contoh: https://www.youtube.com/watch?v=xxxxx atau upload file video)</label>
          <input type="text" name="url_video" placeholder="Masukkan link video..." value="<?= htmlspecialchars($editData['url_video'] ?? '') ?>">
        </div>

        <div class="form-group">
          <label>File Video<?= $editData ? ' (kosongkan jika tidak diganti)' : '' ?></label>
          <input type="file" name="video_file" accept="video/*">
          <?php if (!empty($editData['url_video']) && !preg_match('#^https?://#i', $editData['url_video'])): ?>
          <p style="margin-top:8px;color:var(--gray-300);font-size:0.85rem;">Video lokal saat ini: <?= htmlspecialchars(basename($editData['url_video'])) ?></p>
          <?php endif; ?>
          <?php if ($editData): ?>
          <input type="hidden" name="video_old" value="<?= htmlspecialchars($editData['url_video']) ?>">
          <?php endif; ?>
        </div>

        <div class="form-group">
          <label>Thumbnail / Cover Video<?= $editData ? ' (kosongkan jika tidak diganti)' : '' ?></label>
          <input type="file" name="thumbnail" accept="image/*" onchange="previewImg(this)">
          <?php if (!empty($editData['thumbnail'])): ?>
          <img src="../<?= htmlspecialchars($editData['thumbnail']) ?>" class="img-preview show" id="imgPreview" alt="preview">
          <?php else: ?>
        ...
        <div class="form-actions">
          <button type="button" class="btn-submit" onclick="showConfirm('<?= $editData ? 'Update Video' : 'Tambah Video' ?>', '<?= $editData ? 'Apakah Anda yakin ingin mengupdate video ini?' : 'Apakah Anda yakin ingin menambahkan video ini?' ?>', 'formVideo', false)">
            <?= $editData ? 'Update Video' : 'Simpan Video' ?>
          </button>
          <a href="video.php" class="btn-cancel">Batal</a>
        </div>
      </form>
    </div>
    <?php endif; ?>
    ...
    <div class="section-hdr">
      <h2>Daftar <span>Video Terkait</span></h2>
      <?php if (!$showForm): ?>
      <span style="font-size:0.82rem;color:var(--gray-400);"><?= count($videoList) ?> video tersimpan</span>
      <?php endif; ?>
    </div>
    ...
    <div class="table-wrap">
      ...
      <div class="empty-state">
        ...
        <p>Belum ada video terkait. Klik "Tambah Video Baru" untuk mulai.</p>
      </div>
      <?php else: ?>
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <?php if ($v['thumbnail']): ?>
              <img src="../<?= htmlspecialchars($v['thumbnail']) ?>" class="td-thumb" alt="">
              <?php else: ?>
              <div class="td-no-img">No<br>Image</div>
              <?php endif; ?>
            </td>
            <td style="color:var(--white);font-weight:500;max-width:280px;"><?= htmlspecialchars($v['judul']) ?></td>
            <td><a href="<?= htmlspecialchars($v['url_video']) ?>" target="_blank" style="color:var(--gold); text-decoration:underline;">Lihat Video</a></td>
            <td>
              <div style="display:flex;gap:8px;flex-wrap:wrap;">
                <a href="video.php?edit=<?= $v['id'] ?>" class="btn-edit">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                  Edit
                </a>
                <button class="btn-del" onclick="confirmDelete(<?= $v['id'] ?>, '<?= addslashes(htmlspecialchars($v['judul'])) ?>')">
...
function confirmDelete(id, judul) {
  ...
  document.getElementById('confirmTitle').textContent = 'Hapus Video';
  document.getElementById('confirmMsg').textContent = 'Apakah Anda yakin ingin menghapus video "' + judul + '"? Tindakan ini tidak dapat dibatalkan.';
